;(function(){
	...
	function ImageViewer(options) {
		...
		save.style.lineHeight = "30px";
		save.style.color = "#fff";
		save.style.border = "solid 1px rgba(200,200,200,0.4)";
		save.style.backgroundColor = "rgba(30,30,30,0.4)";
		save.style.borderRadius = "5px";
		save.style.position = "absolute";
		save.style.left = "10px";
		save.style.bottom = "10px";
		save.style.padding = "0 15px";
		save.style.zIndex = 99;
		save.style.display = "none";
		save.innerHTML = "保存到手机";
	...
	ImageViewer.prototype.initEvents = function() {
		...
		this._save.addEventListener("touchstart", function(evt) {
			evt.stopPropagation();
			evt.preventDefault();
			this.style.backgroundColor = "rgba(0,0,0,0.2)";
		});
		this._save.addEventListener("touchend", function(evt) {
			evt.stopPropagation();
			evt.preventDefault();
			this.style.backgroundColor = null;
	...
	ImageViewer.prototype.ontouchend = function(evt) {
			...
			if(!(div._s > 1 && Math.abs(this.index * rect.width + x) < 5
				...
				if(this._moved && !(div._s > 1)) {
					...
					if(this._paths.length >= count) {
						for(var i = this._paths.length - 1, end = Math.max(this._paths.length - count, 0); i > end; i--) {
							path += this._paths[i];
						}
					}
				}			
		...
		} else if(this.onremove && this.edit) {
			this.toggleHeader();
	...
	ImageViewer.prototype.lazyLoading = function(index, showOrigin, silent) {
		...
		if(silent != false) {
			...
			if(index < this.imageObjects.length - 1) {
				...
				window.setTimeout(function() {
					self.lazyLoading(index + 1, false, false); // 自动加载下一组
				}, 3000);
			}
		...
		if(!imageObject.isLoaded) {
			if(source) {
				...
				var spinBorder = document.createElement("div");
				spinBorder.style.position = "absolute";
				spinBorder.style.top = "50%";
				spinBorder.style.left = "50%";
				spinBorder.style.padding = "10px";
				spinBorder.style.webkitTransform = "translate(-50%, -50%)";
				imageObject.div.appendChild(spinBorder);
				
				spinBorder.innerHTML = "<svg width='36px' height='36px' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100' preserveAspectRatio='xMidYMid' class='uil-ring'><rect x='0' y='0' width='100' height='100' fill='none' class='bk'></rect><circle cx='50' cy='50' r='47.5' stroke-dasharray='193.99334635916975 104.45795573186061' stroke='#ED7222' fill='none' stroke-width='5'><animateTransform attributeName='transform' type='rotate' values='0 50 50;180 50 50;360 50 50;' keyTimes='0;0.5;1' dur='1s' repeatCount='indefinite' begin='0s'></animateTransform></circle></svg>";
				img.onload = function() {
					if(img.width < rect.width && img.height < rect.height) {
						img.style.top = (rect.height - img.height) * 0.5 + "px";
						img.style.left = (rect.width - img.width) * 0.5 + "px";
					...
					if(!imageObject.div || !imageObject.div.removeChild) {
						return;
					}
					imageObject.div.removeChild(imageObject.img);
					imageObject.img = img;
					if(self.index == index && imageObject.origin) {
						if(imageObject.isLoadedOrigin) {
							self._origin.style.display = "none";
						} else {
							self._origin.style.display = "block";
						}
					}
					imageObject.div.removeChild(spinBorder);
