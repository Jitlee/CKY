...
//		echo $this->getLastSql();
	...
	public function orderDetailForNotify($orderid) {
		...
		$field = "o.orderId, orderNo, o.orderType, o.createTime, o.mmid, o.shopId, o.isAppraises, shopName,s.wxopenid,s.wxopenid1,s.wxopenid2, replace(s.shopImg, '.', '_thumb.') shopImg, (totalMoney + deliverMoney) AS totalMoney,orderStatus, needPay, payType, GROUP_CONCAT(goodsName ORDER BY og.id) goods";
		$join = 'o inner join __SHOPS__ s on o.shopId = s.shopId inner join __ORDER_GOODS__ og on og.orderId = o.orderId';
		$group = 'o.orderId';
		$list = $this->field($field)->join($join)->where($map)
