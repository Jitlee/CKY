...
class MemberOneCardSyncModel extends BaseModel {
	/*获取用户信息*/
	public function DataSync($cardid)
	{
		$result["status"]=-1;	
	 	$mMember = D('M/Member');
		$MemberItem=$mMember->GetByCardID($cardid);	
		$pTime=$MemberItem["ChangeTime"]+120;	
		//echo $pTime;		
		if($pTime > time())
		{
			return $result;
		...
		{			
			...
			if($onecard)
			{
				...
				if(empty($MemberItem["ChangeTime"])==1)
				{	
	...
	//end 
	/***countlist  我的充次**/
	public function CountlistCheck($uid,$cardid)
	{
		$m = D('M/OneCard');
		$res=$m->GetCountList($cardid,0,30);//可能有多条。		 
 		//用户信息
		$status= $res["status"];
		if($status == 0)
		{
