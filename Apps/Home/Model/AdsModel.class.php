...
class AdsModel extends BaseModel {
	...
	public function getAds($areaId2,$adType){
		$today = date("Y-m-d");
		$data = S('RTC_CACHE_ADS_'.$areaId2."_".$adType);
		if(!$data){
			//获取所在省份
			...
			$data = $this->query($sql);
			S('RTC_CACHE_ADS_'.$areaId2."_".$adType,$data,86400);
		}
		return $data;
	}
	/**
	...
	public function getAdsByCat($areaId2){
		$today = date("Y-m-d");
		$data = S('RTC_CACHE_ADS_CAT_'.$areaId2);
		if(!$data){
			//获取所在省份
			$sql = "select parentId from __PREFIX__areas where areaId=".$areaId2;
			...
			S('RTC_CACHE_ADS_CAT_'.$areaId2,$data,86400);
		}
		return $data;
	}
	/**
