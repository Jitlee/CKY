   ...
   <body class='rtc-page'>
     <form method='post' action='<?php echo U("Admin/Users/queryAccountByPage");?>'>
       ...
       会员类型：<select id='userType' name='userType' class="form-control rtc-ipt-10">
         ...
         账号状态：<select id='userStatus' name='userStatus' class="form-control rtc-ipt-10">
        ...
        <table class="table table-hover table-striped table-bordered rtc-list">
           ...
           <tbody>
            <?php if(is_array($Page['root'])): $i = 0; $__LIST__ = $Page['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
               ...
               <td>
               <div class="dropdown">
               <?php if($vo['userStatus']==0 ): ?><button class="btn btn-danger dropdown-toggle rtc-btn-dropdown"  type="button" data-toggle="dropdown">
					     停用
               ...
               <?php if(in_array('hyzh_04',$RTC_STAFF['grant'])){ ?>
