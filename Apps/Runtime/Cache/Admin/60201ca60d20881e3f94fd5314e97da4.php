...
<html lang="zh-cn">
   ...
   $(function () {
	   ...
	   <?php if(!empty($object["areaId1"])): ?>getAreaList("areaId2","<?php echo ($object['areaId1']); ?>",0,"<?php echo ($object['areaId2']); ?>");<?php endif; ?>
   });
   ...
   function edit(){
	   ...
	   $.post("<?php echo U('Admin/Navs/edit');?>",params,function(data,textStatus){
			var json = RTC.toJson(data);
			if(json.status=='1'){
				Plugins.setWaitTipsMsg({ content:'操作成功',timeout:1000,callback:function(){
				   location.href="<?php echo U('Admin/Navs/index');?>";
				}});
			}else{
				Plugins.closeWindow();
       ...
       <form name="myform" method="post" id="myform" autocomplete="off">
        ...
        <table class="table table-hover table-striped table-bordered rtc-form">
           ...
           <tr>
             ...
             <td>
               <select id='navType'>
                 <option value='0' <?php if($object['navType'] == 0 ): ?>selected<?php endif; ?>>顶部</option>
                 <option value='1' <?php if($object['navType'] == 1 ): ?>selected<?php endif; ?>>底部</option>
               </select>
             </td>
           </tr>
           <tr>
             <th width='150' align='right'>导航名称<font color='red'>*</font>：</th>
             <td><input type='text' id='navTitle' class="form-control rtc-ipt" value='<?php echo ($object["navTitle"]); ?>' maxLength='50'/></td>
           </tr>
           <tr>
           ...
           <tr>
             ...
             <td>
            	<select id="articles" style="display:none;" onchange="changeArticles(this)">
             		<?php if(is_array($articles)): $kv = 0; $__LIST__ = $articles;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($kv % 2 );++$kv;?><option value="<?php echo RTCRootDomain(); echo U('Home/Articles/index/',array('articleId'=>$vo['articleId']));?>"><?php echo ($vo["articleTitle"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
             	</select>
             	<input type='text' id='navUrl' class="form-control rtc-ipt" value='<?php echo ($object["navUrl"]); ?>' maxLength='100'/>
             </td>
           </tr>
           <tr>
             <th align='right'>是否显示<font color='red'>*</font>：</th>
             <td>
             <label><input type='radio' id='isShow0' name='isShow' value='0' <?php if($object['isShow'] == 0): ?>checked<?php endif; ?>>隐藏</label>
             <label><input type='radio' id='isShow1' name='isShow' value='1' <?php if($object['isShow'] == 1): ?>checked<?php endif; ?>>显示</label>
             </td>
           </tr>
           <tr>
             <th align='right'>打开方式<font color='red'>*</font>：</th>
             <td>
             <label><input type='radio' id='isOpen0' name='isOpen' value='0' <?php if($object['isOpen'] == 0): ?>checked<?php endif; ?>>页面跳转</label>
             <label><input type='radio' id='isOpen1' name='isOpen' value='1' <?php if($object['isOpen'] == 1): ?>checked<?php endif; ?>>新窗口打开</label>
             </td>
           </tr>
           <tr>
             <th align='right'>导航排序号：</th>
             <td>
             <input type='text' id='navSort' class="form-control"  value='<?php echo ($object["navSort"]); ?>' style='width:80px' onkeypress="return RTC.isNumberKey(event)" onkeyup="javascript:RTC.isChinese(this,1)" maxLength='8'/>
             </td>
           </tr>
           <tr>
             <td colspan='2' style='padding-left:250px;'>
                 <button type="submit" class="btn btn-success">保&nbsp;存</button>
                 <button type="button" class="btn btn-primary" onclick='javascript:location.href="<?php echo U('Admin/Navs/index');?>"'>返&nbsp;回</button>
             </td>
           </tr>
        </table>
       </form>
