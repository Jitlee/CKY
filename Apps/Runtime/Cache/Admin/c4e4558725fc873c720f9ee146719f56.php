...
<html lang="zh-cn">
   ...
   <body class='rtc-page'>
     <form method='post' action="<?php echo U('Admin/Staffs/index');?>">
       <div class='rtc-tbar'> 
       账号：<input type='text' id='loginName' name='loginName' class='form-control rtc-ipt-10' value='<?php echo ($loginName); ?>'/> 
       姓名：<input type='text' id='staffName' name='staffName' class='form-control rtc-ipt-10' value='<?php echo ($staffName); ?>'/>   
  <button type="submit" class="btn btn-primary glyphicon glyphicon-search">查询</button> 
       <?php if(in_array('zylb_01',$RTC_STAFF['grant'])){ ?>
        ...
        <table class="table table-hover table-striped table-bordered rtc-list">
           ...
           <tbody>
            <?php if(is_array($Page['root'])): $i = 0; $__LIST__ = $Page['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
               ...
               <td>
               <div class="dropdown">
               <?php if($vo['staffStatus']==0 ): ?><button class="btn btn-danger dropdown-toggle rtc-btn-dropdown"  type="button" data-toggle="dropdown">
					    停用
               ...
               <?php if(in_array('zylb_02',$RTC_STAFF['grant'])){ ?>
               ...
               <?php } ?>
               <?php if(in_array('zylb_03',$RTC_STAFF['grant'])){ ?>
