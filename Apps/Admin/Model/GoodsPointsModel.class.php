 ...
 * 抢购
 */
class GoodsPointsModel extends BaseModel {
	...
	/**
     ...
     public function queryByPage(){
        $m = M('goods'); 
	 	...
	 	$sql = "select 
	 				g.*,gc.catName
	 				,p.pointsid,p.points,p.xiangou,p.buyinfo,p.subtitle 				
		 	from __PREFIX__goods g 
			left join __PREFIX__goods_cats gc on g.goodsCatId2=gc.catId			
			inner join __PREFIX__goods_points p on p.goodsId=g.goodsId 
			where goodsFlag=1  ";
...
//		$GoodsGroupId= newid();
		...
		$miaosha["points"] = (int)I("points");
		$miaosha["xiangou"] = (int)I("xiangou");		
		$miaosha["buyinfo"] = I("buyinfo");
	...
	public function edit(){
		...
		if($this->checkEmpty($data,true)){
			...
			if($rs !== false && $rs !== null) {
				//秒杀明细
				$mMiaosha = M('goods_points');
				$pointsid=$miaosha['pointsid'];
				$filter="pointsid='$pointsid'";
